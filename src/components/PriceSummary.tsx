...
export function PriceSummary({
  ...
  return (
    <div className="rounded-2xl border border-black/5 bg-white p-5">
      <p className="text-sm font-bold text-gris-tinta/70">Este es tu precio final</p>
      <p className="text-3xl font-bold text-azul-noche">{formatEuros(total)}</p>

      <ul className="mt-4 space-y-1.5 text-sm text-gris-tinta/75">
        <li className="flex items-center gap-2">
          <span className="text-azul-noche">✓</span>
        ...
        {!!usbCopies && usbCopies > 0 && (
          <li className="flex items-center justify-between gap-2 border-t border-black/5 pt-1.5">
            <span>
              <span className="text-azul-noche">✓</span> Copia USB extra × {usbCopies}
            </span>
      ...
      <p className="mt-4 border-t border-black/10 pt-3 text-xs text-gris-tinta/70">
